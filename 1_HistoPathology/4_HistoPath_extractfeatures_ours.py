...
def extract_features_weighted_attention(model, val_dataloader, device):
    ## Validation
    ...
    for b_idx, batch_dict in enumerate(val_dataloader):
        ...
        attention_weights_list.append(attention_weights.detach().cpu().numpy())
    ...
    attention_weights_list = np.concatenate(attention_weights_list, axis=0)

    case_uniques = []
    features_final = []
    for i, case in enumerate(set(case_list)):
        case_uniques.append(case)
        case_indices = np.array([(x == case) for x in case_list])
        case_features = features_list[case_indices]
        case_attention_weights = attention_weights_list[case_indices]

        # Weight features by attention weights and average
        weighted_features = case_features * case_attention_weights
        feature = np.sum(weighted_features, axis=0) / np.sum(case_attention_weights, axis=0, keepdims=True)
